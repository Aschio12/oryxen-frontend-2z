      ...
      <section className="sticky top-16 z-40 border-b border-white/10 bg-[#030303]/98 backdrop-blur-xl px-4 md:px-6 lg:px-8 py-4 shadow-lg shadow-black/20">
        <div className="mx-auto max-w-[1400px] flex items-center justify-between">
          <div className="flex gap-6 text-sm">
            <div><span className="text-[9px] uppercase tracking-widest text-white/40">Protein</span><p className="mt-1 text-[#FF6B6B]">{plan.macros.protein}g</p></div>
            <div><span className="text-[9px] uppercase tracking-widest text-white/40">Carbs</span><p className="mt-1 text-[#4ECDC4]">{plan.macros.carbs}g</p></div>
            <div><span className="text-[9px] uppercase tracking-widest text-white/40">Fats</span><p className="mt-1 text-[#FFB84D]">{plan.macros.fats}g</p></div>
          </div>
              ...
              {[
                ...
                <div key={idx}>
                  ...
                  <div className="h-3 w-full rounded-full bg-white/10 overflow-hidden border border-white/5">
                    <div className={`h-full bg-gradient-to-r ${macro.gradient} transition-all duration-1000`} style={{ width: `${macro.pct}%` }} />
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="rounded-xl border border-white/10 bg-[#0A0A0A] p-8 mb-12 motion-reveal">
            ...
            <div className="space-y-4">
              {plan.meals.map((meal, idx) => (
                <div key={idx} className="rounded-lg border border-white/10 bg-white/5 p-5 hover:border-[#C5A059]/30 transition-all hover:bg-white/[0.07]">
            ...
            <div className="space-y-4">
              {plan.guidelines.map((guideline, idx) => (
                <div key={idx} className="flex gap-4 items-start">
                  <span className="flex h-7 w-7 items-center justify-center rounded-full bg-[#C5A059]/20 text-xs text-[#C5A059] font-semibold flex-shrink-0">{idx + 1}</span>
                  <p className="text-sm font-light text-white/70 leading-relaxed pt-0.5">{guideline}</p>
                </div>
              ))}
            </div>
          </div>
